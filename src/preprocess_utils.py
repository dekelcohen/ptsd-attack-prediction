...
class PreprocessUtils:
    ...
    def __init__(self, cfg):
        ...
        self.file_formats = {
            'polar_csv': {'header': 'infer', 'sep': ';', 'sig_type_globexpr': '/**/*{sig_type}.txt',
                          'preprocess_mtd': self.preprocess_polar},
            'empatica_csv': {'header': None, 'sep': ' ', 'sig_type_globexpr': '/**/*{sig_type}.csv',
                             'preprocess_mtd': self.preprocess_empatica},
            'empatica_avro': {'header': None, 'sep': ' ', 'sig_type_globexpr': '/**/*{sig_type}.csv',
                             'preprocess_mtd': self.preprocess_empatica},
        }
    ...
    def read_sensor_files(self, sig_type, fmt, root_path):
        ...
        if len(sigs_samplings[1]) > 1 and stdev(sigs_samplings[1]) > 0:
    ...
    def read_preprocess_labels(self, path):
        ...
        try:
            ...
            df_tags[self.cfg.TIMESTAMP_COL] = df_tags_raw['startLocalTime'].astype('datetime64').dt.tz_localize(
                'UTC')
    ...
    def add_windows_indices(self, tags_indices, df_sig, n_windows, sampling_rate, window_len):
        ...
        def create_before_after_indxs(tag_ind):
            ...
            before_idxs = range(ind - wnd_len_ind, max(df_sig.index.min(), ind - (n_windows + 1) * wnd_len_ind),
                                -wnd_len_ind)
            after_idxs = range(ind + wnd_len_ind, min(df_sig.index.max(), ind + (n_windows + 1) * wnd_len_ind),
                               wnd_len_ind)
